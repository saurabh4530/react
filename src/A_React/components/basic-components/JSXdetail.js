...
? 2. JSX is an Expression
   ...
   -VVVIMP - Usage in Statements: This means you can use JSX inside `if` statements, `for` loops, 
...
? 8. User-Defined Components Must Be Capitalized
    ...
    - Naming Convention: When an element type starts with a lowercase letter, 
    React treats it as a built-in component (like `<div>` or `<span>`) and passes the resulting string to HTML.
     For user-defined components, you must use uppercase.
...
- HTML-like JSX tags (e.g., `<div>`, `<span>`, `<a>`) are transformed into React elements which are objects describing what should appear on the screen.
 These are distinctly different from strings.
...
- Besides using ternary operators and logical `&&`, conditional rendering in JSX can also be achieved using `null` values for conditional outputs. 
This is particularly useful in `&&` conditions where you might not want any output if a condition fails.
...
- JSX allows you to directly assign event handlers using camelCase property names such as `onClick`, `onHover`, etc.
 These handlers are set directly on components or HTML elements and can reference functions defined in JavaScript.
...
- Sometimes you need to return multiple elements from a component. JSX fragments allow you to group a list of children without adding extra nodes to the DOM.
This is done using `<React.Fragment>` or the shorthand `<>` syntax.
